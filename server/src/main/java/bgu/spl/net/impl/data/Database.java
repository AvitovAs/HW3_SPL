...
public class Database {
    ...
    private void loadUserFromDB(String username) {
        String sql = "SELECT password FROM users WHERE username = '" + escapeSql(username) + "'";
        String result = executeSQL(sql);
        
        // Expected format from Python server: "SUCCESS|('thePassword',)"
        if (result.startsWith("SUCCESS")) {
            String[] parts = result.split("\\|");
            if (parts.length > 1) {
                String row = parts[1].trim(); 
                // Basic parsing of Python tuple string representation: ('password',)
                // We extract the string between the first and last single quotes.
                int firstQuote = row.indexOf("'");
                int lastQuote = row.lastIndexOf("'");
                
                if (firstQuote != -1 && lastQuote > firstQuote) {
                    String dbPassword = row.substring(firstQuote + 1, lastQuote);
                    
                    // Create user with DB password and add to memory
                    // ConnectionID is -1 initially; it gets set upon successful login in userExistsCase
                    User user = new User(-1, username, dbPassword);
